---
title: "Calculating fold-change in phosphorylation over time"
output:
---
...
#
# Here we take all normalised phosphorylation data and calculate the fold-change between maximum and minimum values.
...
#
# INPUTS = normalised phospho data (.csv)
...
WT <- apply(WT_data_start[, 6:ncol(WT_data_start)], 1, function(x){max(x)/min(x)})
CKO <- apply(CKO_data_start[, 6:ncol(CKO_data_start)], 1, function(x){max(x)/min(x)})
...
ggsave(plot=pd, "Phosphoproteome prob densities.pdf", width=8, height = 5)
